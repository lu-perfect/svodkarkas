import type { HTMLAttributeAnchorTarget } from "react";

import Icon from "components/UI/Icon";
import ListItem from "components/UI/ListItem";
import Link from "components/UI/Link";

export type SocialLinkProps = {
  id: string;
  href: string;
  target?: HTMLAttributeAnchorTarget;
  rel?: string;

  tag?: keyof JSX.IntrinsicElements;
}

const SocialLink = ({ id, href } : SocialLinkProps) => (
  <ListItem className="social-link">
    <Link target="_blank" href={href} className="social-link__link">
      {(() => {
        switch (id) {
          case 'vk':
            return (
              <Icon viewBox="0 0 24 24" width="1.5em" height="1.5em" className="social-link__icon">
                <path d="M21.579 6.855c.14-.465 0-.806-.662-.806h-2.193c-.558 0-.813.295-.953.619 0 0-1.115 2.719-2.695 4.482-.51.513-.743.675-1.021.675-.139 0-.341-.162-.341-.627V6.855c0-.558-.161-.806-.626-.806H9.642c-.348 0-.558.258-.558.504 0 .528.79.65.871 2.138v3.228c0 .707-.127.836-.407.836-.743 0-2.551-2.729-3.624-5.853-.209-.607-.42-.852-.98-.852H2.752c-.627 0-.752.295-.752.619 0 .582.743 3.462 3.461 7.271 1.812 2.601 4.363 4.011 6.687 4.011 1.393 0 1.565-.313 1.565-.853v-1.966c0-.626.133-.752.574-.752.324 0 .882.164 2.183 1.417 1.486 1.486 1.732 2.153 2.567 2.153h2.192c.626 0 .939-.313.759-.931-.197-.615-.907-1.51-1.849-2.569-.512-.604-1.277-1.254-1.51-1.579-.325-.419-.231-.604 0-.976.001.001 2.672-3.761 2.95-5.04z"/>
              </Icon>
            )
          case 'yt':
            return (
              <Icon viewBox="0 0 461 461" width="1.5em" height="1.5em" className="social-link__icon">
                <path d="M365.257,67.393H95.744C42.866,67.393,0,110.259,0,163.137v134.728   c0,52.878,42.866,95.744,95.744,95.744h269.513c52.878,0,95.744-42.866,95.744-95.744V163.137   C461.001,110.259,418.135,67.393,365.257,67.393z M300.506,237.056l-126.06,60.123c-3.359,1.602-7.239-0.847-7.239-4.568V168.607   c0-3.774,3.982-6.22,7.348-4.514l126.06,63.881C304.363,229.873,304.298,235.248,300.506,237.056z"/>
              </Icon>
            );
          case 'tg':
            return (
              <Icon viewBox="0 0 48 48" width="1.5em" height="1.5em" className="social-link__icon">
                <path d="M41.4193 7.30899C41.4193 7.30899 45.3046 5.79399 44.9808 9.47328C44.8729 10.9883 43.9016 16.2908 43.1461 22.0262L40.5559 39.0159C40.5559 39.0159 40.3401 41.5048 38.3974 41.9377C36.4547 42.3705 33.5408 40.4227 33.0011 39.9898C32.5694 39.6652 24.9068 34.7955 22.2086 32.4148C21.4531 31.7655 20.5897 30.4669 22.3165 28.9519L33.6487 18.1305C34.9438 16.8319 36.2389 13.8019 30.8426 17.4812L15.7331 27.7616C15.7331 27.7616 14.0063 28.8437 10.7686 27.8698L3.75342 25.7055C3.75342 25.7055 1.16321 24.0823 5.58815 22.459C16.3807 17.3729 29.6555 12.1786 41.4193 7.30899Z" />
              </Icon>
            );
          default:
            throw new Error('unknown social provider');
        }
      })()}
    </Link>
  </ListItem>
);

export default SocialLink;
